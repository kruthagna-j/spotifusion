  ...
  return (
    ...
    >
      <div className="flex items-center justify-center w-6">
        <span className="group-hover:hidden text-sm" aria-hidden="true">{index + 1}</span>
        <button
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <button
            type="button"
            onClick={() => { player.playTrack(track, contextTracks); player.openNowPlaying() }}
            className={`text-sm truncate text-left hover:underline ${isCurrent ? 'text-brand' : 'text-text'}`}
            title="Open Now Playing"
          >{track.title}</button>
          <div className="text-xs text-text-subdued truncate flex items-center gap-1">
            {track.artist ? <Link to={`/artist/${encodeURIComponent(track.artist)}`} state={{ tracks: contextTracks, name: track.artist }} onClick={(e) => e.stopPropagation()} className="truncate hover:text-white hover:underline">{track.artist}</Link> : 'Unknown artist'}
            {track.album && <> <span className="text-text-subdued">·</span> <Link to={`/album/${encodeURIComponent(track.album)}`} state={{ tracks: contextTracks, name: track.album }} onClick={(e) => e.stopPropagation()} className="truncate hover:text-white hover:underline">{track.album}</Link></>}
            {track.trusted && <BadgeCheck size={12} className="text-brand shrink-0" aria-label="Verified source" />}
          </div>
        </div>
      </div>
