...
function Artwork({ track }) {
 const src = getArtwork(track, 'small')
 const fallback = getArtwork(track, 'medium')
 return <img src={src || fallback || ''} alt="" loading="lazy" decoding="async" className="w-10 h-10 rounded object-cover shrink-0 bg-surface-highlight" onError={(e)=>{ if (fallback && e.currentTarget.src !== fallback) e.currentTarget.src=fallback; else e.currentTarget.style.visibility='hidden' }} />
}
