...
export default function PreferenceWizard({ initialLanguages = [], initialArtists = [], onFinish, onCancel }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const queries = languages.flatMap((lang) => [
        `${lang} popular singers`,
        `${lang} playback singers`,
        `${lang} music artists`,
      ])
      const results = await Promise.allSettled(queries.map((q) => searchMusicPage(q, { category: 'artists', batch: 1 })))
      if (cancelled) return
      ...
      // A much larger pool so users can choose many singers; search remains available for anyone not in the suggestions.
      setSuggestedArtists(merged.slice(0, 72))
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        setArtistResults((page.results || []).filter((x) => x.resultType === 'artist').slice(0, 18))
  ...
  function finish() {
    onFinish({ languages, language: languages[0], favoriteArtists: artists, preferredSongs: [], onboardingComplete: true })
  }
  ...
  return <div className="sf-onboarding-page px-3 py-4 sm:px-6">
    <div className="sf-onboarding-card w-full max-w-3xl max-h-[calc(100vh-2rem)] overflow-y-auto">
      ...
      {step === 1 && <section>
        ...
        <p className="text-sm text-text-muted mb-5">Choose as many as you want. These choices control the artists and songs recommended on your Home screen.</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">{LANGUAGES.map((language) => {
          const selected = languages.includes(language)
          return <button key={language} type="button" onClick={() => toggleLanguage(language)} className={`px-4 py-3 rounded-xl border text-sm font-bold text-left ${selected ? 'border-brand bg-brand/10 text-brand' : 'border-white/10 bg-white/[.03] text-text hover:bg-white/[.07]'}`}><span className="flex items-center justify-between gap-2">{language}{selected && <Check size={16}/>}</span></button>
        })}</div>
      ...
      {step === 2 && <section>
        ...
        <div className="flex gap-2 mt-5"><button onClick={() => setStep(1)} className="sf-secondary-button flex-1"><ArrowLeft size={16}/> Back</button><button disabled={!artists.length} onClick={finish} className="sf-primary-button flex-1 disabled:opacity-40">Start listening <ArrowRight size={17}/></button></div>
      </section>}
