    ...
    <div className="fixed inset-0 z-[95] md:hidden now-playing-reference overflow-y-auto">
      ...
      <div className="px-1">
        ...
        <div className="ref-tool-row mt-5">
          <button className="ref-tool" onClick={() => player.navigate?.('/equalizer')} aria-label="Equalizer"><SlidersHorizontal size={18}/></button>
        ...
        <div className="ref-controls">
          <button className="ref-control" onClick={player.toggleShuffle} aria-label="Shuffle"><Shuffle size={19}/></button>
          ...
          <button className="ref-control" onClick={player.cycleRepeat} aria-label="Repeat">{player.repeatMode === 'one' ? <Repeat1 size={19}/> : <Repeat size={19}/>}</button>
        </div>
        ...
        {panel === 'lyrics' && <div className="rounded-3xl bg-white/70 border border-white/80 p-5 shadow-xl mb-6"><div className="font-bold mb-2">Lyrics</div><p className="text-sm text-[#707487]">Lyrics remain available through the full lyrics panel. Close this view to return to the player.</p></div>}
      </div>
    </div>
  )
}
