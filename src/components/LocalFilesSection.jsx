...
export default function LocalFilesSection() {
  ...
  const [scanStatus, setScanStatus] = useState(null) // { name, found } while actively scanning
  ...
  async function handleReconnect(folder) {
    ...
    try {
      await scanFolder(folder.id, folder.handle, (count) =>
        setScanStatus((s) => (s ? { ...s, found: count } : s))
      )
      refreshFolders()
      refresh()
    } finally {
  ...
  async function handleRemoveFolder(folder) {
    await removeMusicFolder(folder.id)
    refreshFolders()
    refresh()
  }
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-2">
          {SUPPORTS_FOLDER_ACCESS && (
            <button
              ...
              disabled={!!scanStatus}
              aria-label="Add a music folder — discovers every song in it automatically"
              className="flex items-center gap-2 bg-brand hover:bg-brand-hover disabled:opacity-60 text-black text-xs font-bold px-3 py-1.5 rounded-full shrink-0"
            >
              <FolderPlus size={14} />
              Add Music Folder
            </button>
          ...
          <button
            ...
            aria-label="Add individual audio files from this device"
          ...
          >
            <Upload size={14} />
            {uploading ? 'Adding…' : SUPPORTS_FOLDER_ACCESS ? 'Add Files' : 'Add to Device'}
          </button>
        </div>
      ...
      <p className="text-xs text-text-subdued mb-3">
        Your local music stays on your device — Spotifusion never uploads your local audio files.
        {SUPPORTS_FOLDER_ACCESS
          ? ' Add a whole folder once and every supported song inside it (including subfolders) is discovered automatically.'
          : ' Supports MP3, WAV, M4A, AAC, OGG, FLAC, and WEBM.'}
      </p>

      {scanStatus && (
        <div className="flex items-center gap-2 text-sm bg-surface-highlight rounded-md px-3 py-2 mb-3">
          <RefreshCw size={14} className="animate-spin shrink-0" />
          Scanning {scanStatus.name}… found {scanStatus.found} song{scanStatus.found === 1 ? '' : 's'}
        </div>
      )}
      ...
      {folders.length > 0 && (
        <div className="flex flex-col gap-1.5 mb-4">
          {folders.map((folder) => (
            <div
              key={folder.id}
              className="flex items-center gap-3 bg-surface-highlight rounded-md px-3 py-2"
            >
              <Folder size={16} className="text-text-muted shrink-0" />
              <span className="text-sm truncate flex-1">{folder.name}</span>
              {folder.permission === 'granted' ? (
                <span className="text-[11px] text-brand font-semibold shrink-0">Connected</span>
              ) : (
                <button
                  onClick={() => handleReconnect(folder)}
                  className="flex items-center gap-1 text-[11px] font-semibold text-yellow-500 hover:text-yellow-400 shrink-0"
                >
                  <AlertTriangle size={12} /> Reconnect Music Folder
                </button>
              )}
              <button
      ...
      {songs.length > 0 && persisted === false && (
        <button
          onClick={requestPersistentStorage}
          className="flex items-center gap-2 text-xs text-yellow-500 bg-yellow-500/10 hover:bg-yellow-500/20 rounded-md px-3 py-2 mb-3"
        >
          <ShieldCheck size={14} className="shrink-0" />
          The browser may clear these files under storage pressure — tap to grant permanent storage permission.
        </button>
      )}
      ...
      {songs.length > 0 && (
        <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
          ...
          <span className="text-xs text-text-subdued shrink-0">
      ...
      {songs.length === 0 && !scanStatus && (
        <div className="text-center py-16">
          <FolderOpen size={40} className="mx-auto mb-3 text-text-subdued" aria-hidden="true" />
          <p className="text-text-muted text-sm">No local music yet — add a folder or a few files.</p>
        </div>
      )}

      {songs.length > 0 && visible.length === 0 && (
        <p className="text-xs text-text-subdued">No local music matches "{query}".</p>
      )}
          ...
          {visible.map((song, i) => (
            <div key={song.id} className="group flex items-center">
              ...
              <button
                ...
                aria-label={`Remove ${song.title} from this device`}
