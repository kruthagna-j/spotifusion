    ...
    try {
      ...
      if (err.code === 'auth/requires-recent-login') {
        try {
          ...
          await deleteCurrentUserAccount()
          onClose()
        } catch { setError('Could not verify your identity. Please try again.') }
      } else setError('Something went wrong deleting your account. Please try again.')
    } finally { setDeleting(false) }
  }

  const content = <>
    <div className="fixed inset-0 z-[2000] bg-black/70 md:hidden" aria-hidden="true" onClick={onClose} />
    <div ref={panelRef} role="dialog" aria-modal="true" aria-label="Account menu" className="fixed inset-y-0 right-0 z-[2001] w-[min(88vw,380px)] bg-surface-elevated shadow-panel overflow-y-auto overscroll-contain md:inset-y-auto md:top-[68px] md:right-6 md:w-80 md:max-w-none md:rounded-xl md:border md:border-border">
      <div className="flex items-center justify-between p-4 border-b border-border"><p className="font-bold text-sm">Account</p><button onClick={onClose} aria-label="Close account menu" className="text-text-muted hover:text-text"><X size={18}/></button></div>
      <div className="p-4 flex items-center gap-3 border-b border-border">{user.photoURL ? <img src={user.photoURL} alt="" className="w-14 h-14 rounded-full"/> : <div className="w-14 h-14 rounded-full bg-brand flex items-center justify-center"><User size={24} className="text-black"/></div>}<div className="min-w-0"><p className="font-semibold truncate">{user.displayName || 'Spotifusion user'}</p><p className="text-xs text-text-subdued truncate">{user.email}</p></div></div>
      <div className="p-2">
