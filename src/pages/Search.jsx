...
export default function Search() {
  ...
  const initialCache = loadCache(locationQuery, urlCategory)
  ...
  const loadPage = useCallback(async (qValue, categoryValue, pageNumber) => {
  ...
  // The URL is the single source of truth. Category clicks only change the URL;
  // this effect then performs exactly one request for the new category. This
  // prevents the old implementation's race where selectCategory() started a
  // request and the URL effect immediately aborted that same request.
  useEffect(() => {
    if (!user) return
    const nextQ = locationQuery
    const key = `${nextQ}::${urlCategory}`
    if (lastUrlKeyRef.current === key) return
    lastUrlKeyRef.current = key

    const cached = loadCache(nextQ, urlCategory)
    abortRef.current?.abort()
    setQuery(nextQ); setSubmittedQuery(nextQ); setCategory(urlCategory)
    setResults(cached?.results || []); setBatch(cached?.batch || 0); setHasMore(!!cached?.hasMore); setError(null)

    if (nextQ && !cached?.results?.length) loadPage(nextQ, urlCategory, 1)
  }, [user, locationQuery, urlCategory, loadPage])
  ...
  const submitSearch = useCallback((value = query, nextCategory = category) => {
    ...
    setHistory(addHistory(q)); setQuery(q); setError(null)
    navigate(`/search?q=${encodeURIComponent(q)}&category=${encodeURIComponent(nextCategory)}`)
  }, [query, category, navigate])

  function selectCategory(next) {
    if (next === urlCategory) return
    const q = submittedQuery || query
    setError(null)
    // Do not call loadPage here. The URL effect above owns fetching and will
    // not abort its own request.
    navigate(q ? `/search?q=${encodeURIComponent(q)}&category=${encodeURIComponent(next)}` : `/search?category=${encodeURIComponent(next)}`, { replace: true })
  }

  function clearSearch() {
    abortRef.current?.abort()
    setQuery(''); setSubmittedQuery(''); setResults([]); setBatch(0); setHasMore(false); setError(null); setLoading(false); setLoadingMore(false)
