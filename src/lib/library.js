...
export function updatePlaylistDetails(uid, playlistId, { name, description }) {
  const ref = doc(db, `${uidPath(uid)}/playlists/${playlistId}`)
  return setDoc(ref, { name, description, updatedAt: serverTimestamp() }, { merge: true })
}
...
export function addTrackToPlaylist(uid, playlistId, track) {
  ...
  return setDoc(
    ref,
    {
      ...
      [`tracks.${track.id}`]: { ...track, addedAt: Date.now() },
      updatedAt: serverTimestamp(),
    },
    { merge: true }
  )
...
// Persists a custom track order (drag/sort-adjacent use) by rewriting the
// full trackIds array — Firestore has no "reorder array" primitive.
export function reorderPlaylistTracks(uid, playlistId, orderedTrackIds) {
  const ref = doc(db, `${uidPath(uid)}/playlists/${playlistId}`)
  return setDoc(ref, { trackIds: orderedTrackIds, updatedAt: serverTimestamp() }, { merge: true })
}
