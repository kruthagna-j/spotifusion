...
open class NeonActivity : AppCompatActivity() {
    ...
    private fun miniPlayer(): LinearLayout = LinearLayout(this).apply {
        ...
        miniPlay = label(if (playing) "Ⅱ" else "▶", 18f, ink, true).apply { gravity = Gravity.CENTER; setOnClickListener { playing = !playing; updateMini() } }; addView(miniPlay, LinearLayout.LayoutParams(dp(43), dp(50)))
    ...
    private fun miniSuggestion(): LinearLayout = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(9), dp(6), dp(7), dp(6)); this.background = bg(alpha(Color.WHITE, 235), 18, alpha(Color.WHITE, 245)); val art = FrameLayout(this@NeonActivity).apply { this.background = gradient(blue, indigo, 13); addView(label("◉", 19f, Color.WHITE, true).apply { gravity = Gravity.CENTER }) }; addView(art, LinearLayout.LayoutParams(dp(48), dp(48))); val info = LinearLayout(this@NeonActivity).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(10), 0, 0, 0) }; info.addView(label("Glass Architecture", 11.5f, ink, true)); info.addView(label("Nordic Echoes", 9.5f, muted).apply { setPadding(0, dp(3), 0, 0) }); addView(info, LinearLayout.LayoutParams(0, -1, 1f)); addView(label("▶", 15f, Color.WHITE, true).apply { gravity = Gravity.CENTER; this.background = bg(blue, 15); setPadding(dp(6), dp(5), dp(6), dp(5)); setOnClickListener { selectTrack("Glass Architecture", "Astral Pulse") } }, LinearLayout.LayoutParams(dp(42), dp(42))) }

    private fun showSearch() { currentTab = 1; headerTitle.text = "Search"; val c = column(); c.addView(label("Find your sound", 26f, ink, true).apply { setPadding(dp(2), dp(7), 0, dp(14)) }); val input = EditText(this).apply { hint = "Songs, artists, albums..."; textSize = 13f; setSingleLine(true); setPadding(dp(14), 0, dp(14), 0); background = bg(Color.WHITE, 18, alpha(Color.WHITE, 245)) }; c.addView(input, LinearLayout.LayoutParams(-1, dp(50)).apply { bottomMargin = dp(15) }); section(c, "Browse all"); arrayOf("Made For You", "New Releases", "Charts", "Mood", "Genres").forEach { name -> c.addView(label(name, 12f, ink, true).apply { background = bg(alpha(Color.WHITE, 225), 17); setPadding(dp(14), dp(15), dp(14), dp(15)) }, LinearLayout.LayoutParams(-1, dp(52)).apply { bottomMargin = dp(7) }) }; clearAndShow(scrollPage(c)); refreshNav() }

    private fun showNowPlaying() { headerTitle.text = "Now Playing"; val c = column(); c.gravity = Gravity.CENTER_HORIZONTAL; c.addView(label("NOW PLAYING", 9f, blue, true).apply { setPadding(0, dp(10), 0, dp(5)) }); c.addView(label(currentSong, 23f, ink, true)); c.addView(label(currentArtist, 11f, muted).apply { setPadding(0, dp(5), 0, dp(14)) }); val disc = FrameLayout(this).apply { this.background = bg(Color.rgb(252, 253, 255), 999, alpha(blue, 90)); elevation = dp(5).toFloat() }; val vinyl = FrameLayout(this).apply { this.background = bg(Color.rgb(25, 27, 34), 999, alpha(Color.WHITE, 40)); addView(label("◉", 42f, blue, true).apply { gravity = Gravity.CENTER }) }; disc.addView(vinyl, FrameLayout.LayoutParams(dp(214), dp(214), Gravity.CENTER)); c.addView(disc, LinearLayout.LayoutParams(dp(274), dp(274)).apply { topMargin = dp(8); bottomMargin = dp(22) }); c.addView(label("0:00                                      3:42", 9f, muted).apply { gravity = Gravity.CENTER }); val seek = SeekBar(this).apply { max = 1000; progress = 0; setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener { override fun onProgressChanged(bar: SeekBar?, value: Int, fromUser: Boolean) { if (fromUser && PlaybackController.duration() > 0) PlaybackController.seek((PlaybackController.duration() * value / 1000f).toLong()) }; override fun onStartTrackingTouch(bar: SeekBar?) = Unit; override fun onStopTrackingTouch(bar: SeekBar?) = Unit }); progressTintList = android.content.res.ColorStateList.valueOf(blue); thumbTintList = android.content.res.ColorStateList.valueOf(blue) }; c.addView(seek, LinearLayout.LayoutParams(-1, dp(40))); val actions = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER; setPadding(0, dp(7), 0, 0) }; actions.addView(label(if (shuffle) "🔀" else "⇄", 18f, if (shuffle) blue else muted).apply { setOnClickListener { shuffle = !shuffle; PlaybackController.setShuffle(shuffle); setTextColor(if (shuffle) blue else muted) } }, LinearLayout.LayoutParams(dp(45), dp(55))); actions.addView(label("|◀", 21f, ink, true).apply { gravity = Gravity.CENTER; setOnClickListener { PlaybackController.previous() } }, LinearLayout.LayoutParams(dp(50), dp(55))); actions.addView(label(if (playing) "Ⅱ" else "▶", 24f, Color.WHITE, true).apply { gravity = Gravity.CENTER; this.background = bg(blue, 20); setOnClickListener { PlaybackController.toggle(); playing = PlaybackController.isPlaying(); updateMini(); text = if (playing) "Ⅱ" else "▶" } }, LinearLayout.LayoutParams(dp(64), dp(58))); actions.addView(label("▶|", 21f, ink, true).apply { gravity = Gravity.CENTER; setOnClickListener { PlaybackController.next() } }, LinearLayout.LayoutParams(dp(50), dp(55))); actions.addView(label("↻", 21f, if (repeat) blue else muted, true).apply { setOnClickListener { repeat = !repeat; PlaybackController.setRepeat(repeat); setTextColor(if (repeat) blue else muted) } }, LinearLayout.LayoutParams(dp(45), dp(55))); c.addView(actions); val volumeLabel = label("VOLUME", 9f, muted, true); c.addView(volumeLabel, LinearLayout.LayoutParams(-1, dp(25))); val volume = SeekBar(this).apply { max = 100; progress = 75; progressTintList = android.content.res.ColorStateList.valueOf(blue); thumbTintList = android.content.res.ColorStateList.valueOf(blue); setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener { override fun onProgressChanged(bar: SeekBar?, value: Int, fromUser: Boolean) { if (fromUser) PlaybackController.setVolume(value / 100f) }; override fun onStartTrackingTouch(bar: SeekBar?) = Unit; override fun onStopTrackingTouch(bar: SeekBar?) = Unit }) }; c.addView(volume, LinearLayout.LayoutParams(-1, dp(42))); clearAndShow(scrollPage(c)) }

    private fun showLibrary() {
        currentTab = 2
        headerTitle.text = "Your Library"
        val c = column()
        c.addView(label("Your Library", 26f, ink, true).apply { setPadding(dp(2), dp(7), 0, dp(5)) })
        c.addView(label(if (localTracks.isEmpty()) "No music found yet" else "${localTracks.size} songs on this device", 11.5f, muted).apply { setPadding(dp(2), 0, 0, dp(12)) })
        val actions = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL }
        actions.addView(label("↻  SCAN MUSIC", 10.5f, Color.WHITE, true).apply { gravity = Gravity.CENTER; background = bg(blue, 14); setPadding(dp(12), dp(9), dp(12), dp(9)); setOnClickListener { localTracks = runCatching { LocalMusicScanner.scan(this@NeonActivity) }.getOrElse { emptyList() }; Toast.makeText(this@NeonActivity, "Found ${localTracks.size} music track${if (localTracks.size == 1) "" else "s"}", Toast.LENGTH_SHORT).show(); showLibrary() } }, LinearLayout.LayoutParams(-2, dp(38)))
        actions.addView(label("PLAY ALL", 10.5f, blue, true).apply { gravity = Gravity.CENTER; background = bg(alpha(blue, 22), 14); setPadding(dp(14), dp(9), dp(14), dp(9)); setOnClickListener { playLocalTracks(0) } }, LinearLayout.LayoutParams(-2, dp(38)).apply { leftMargin = dp(8) })
        c.addView(actions, LinearLayout.LayoutParams(-1, dp(42)).apply { bottomMargin = dp(13) })
        if (localTracks.isEmpty()) {
            val empty = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER; setPadding(dp(25), dp(35), dp(25), dp(35)); background = bg(alpha(Color.WHITE, 220), 22) }
            empty.addView(label("♫", 42f, blue, true).apply { gravity = Gravity.CENTER })
            empty.addView(label("Your device music will appear here", 14f, ink, true).apply { gravity = Gravity.CENTER; setPadding(0, dp(14), 0, dp(5)) })
            empty.addView(label("Allow music access, then tap Scan Music.", 11f, muted).apply { gravity = Gravity.CENTER })
            c.addView(empty, LinearLayout.LayoutParams(-1, dp(190)))
        } else localTracks.forEachIndexed { index, track -> localTrackItem(c, track, index) }
        clearAndShow(scrollPage(c)); refreshNav()
    }

    private fun localTrackItem(c: LinearLayout, track: LocalTrack, index: Int) {
        val row = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(7), dp(7), dp(7), dp(7)); background = bg(alpha(Color.WHITE, 225), 17); setOnClickListener { playLocalTracks(index) } }
        val art = FrameLayout(this).apply { background = gradient(if (index % 2 == 0) blue else indigo, cyan, 14); addView(label("♫", 22f, Color.WHITE, true).apply { gravity = Gravity.CENTER }) }
        row.addView(art, LinearLayout.LayoutParams(dp(54), dp(54)))
        val info = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(11), 0, dp(4), 0) }
        info.addView(label(track.title, 12.5f, ink, true).apply { maxLines = 1; ellipsize = android.text.TextUtils.TruncateAt.END })
        info.addView(label(track.artist, 10.5f, muted).apply { maxLines = 1; ellipsize = android.text.TextUtils.TruncateAt.END; setPadding(0, dp(4), 0, 0) })
        info.addView(label(track.album, 9.5f, muted).apply { maxLines = 1; ellipsize = android.text.TextUtils.TruncateAt.END; setPadding(0, dp(3), 0, 0) })
        row.addView(info, LinearLayout.LayoutParams(0, -2, 1f))
        row.addView(label("▶", 15f, blue, true).apply { gravity = Gravity.CENTER }, LinearLayout.LayoutParams(dp(40), dp(54)))
        c.addView(row, LinearLayout.LayoutParams(-1, dp(70)).apply { bottomMargin = dp(7) })
    }

    private fun playLocalTracks(startIndex: Int) {
        if (localTracks.isEmpty()) { Toast.makeText(this, "No music found on this device.", Toast.LENGTH_SHORT).show(); return }
        val index = startIndex.coerceIn(0, localTracks.lastIndex)
        val track = localTracks[index]
        currentSong = track.title; currentArtist = track.artist; playing = true; updateMini()
        PlaybackController.playTracks(localTracks, index)
        showNowPlaying()
    }
    ...
    private fun showSettings() { currentTab = 4; headerTitle.text = "Settings"; val c = column(); c.addView(label("Settings", 26f, ink, true).apply { setPadding(dp(2), dp(7), 0, dp(14)) }); settingRow(c, "Offline music", "On", true); settingRow(c, "Shake to change song", "On", true); settingRow(c, "High quality audio", "On", true); settingRow(c, "Gapless playback", "On", true); settingRow(c, "Theme", "Light", true); settingRow(c, "Sleep timer", "Off", true); clearAndShow(scrollPage(c)); refreshNav() }
    private fun settingRow(c: LinearLayout, title: String, value: String, enabled: Boolean) { val row = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(13), dp(10), dp(10), dp(10)); background = bg(alpha(Color.WHITE, 225), 17) }; val info = LinearLayout(this@NeonActivity).apply { orientation = LinearLayout.VERTICAL }; info.addView(label(title, 12f, ink, true)); info.addView(label(value, 9.5f, muted).apply { setPadding(0, dp(4), 0, 0) }); row.addView(info, LinearLayout.LayoutParams(0, -2, 1f)); row.addView(label(if (enabled) "●" else "○", 18f, if (enabled) blue else muted)); c.addView(row, LinearLayout.LayoutParams(-1, dp(64)).apply { bottomMargin = dp(7) }) }
