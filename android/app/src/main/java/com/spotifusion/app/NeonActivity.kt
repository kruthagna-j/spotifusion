...
open class NeonActivity : AppCompatActivity() {
    ...
    private fun showHome() {
        ...
        section(c, "Recently Played"); val hs = HorizontalScrollView(this).apply { isHorizontalScrollBarEnabled = false }; val row = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }; listOf("Glass Architecture" to "Astral Pulse", "Astral Pulse" to "Lunar Drift", "Midnight Drive" to "Neon Skyline", "Ocean Bloom" to "Cobalt Waves").forEach { recentTrack(row, it.first, it.second) }; hs.addView(row); c.addView(hs, LinearLayout.LayoutParams(-1, dp(151)).apply { bottomMargin = dp(12) }); section(c, "Made For You"); val mixes = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }; mixCard(mixes, "Focus\nFlow", blue, cyan); mixCard(mixes, "Chill\nSpace", indigo, pink); mixCard(mixes, "Energy\nBoost", orange, pink); c.addView(mixes, LinearLayout.LayoutParams(-1, dp(142))); c.addView(miniSuggestion(), LinearLayout.LayoutParams(-1, dp(62)).apply { topMargin = dp(9) }); clearAndShow(scrollPage(c))
    }

    private fun recentTrack(parent: LinearLayout, title: String, artist: String) { val card = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(7), dp(7), dp(7), dp(4)); setOnClickListener { selectTrack(title, artist) } }; val art = FrameLayout(this).apply { this.background = gradient(blue, Color.rgb(215, 224, 255), 16); addView(label("◉", 28f, Color.WHITE, true).apply { gravity = Gravity.CENTER }) }; card.addView(art, LinearLayout.LayoutParams(dp(102), dp(102))); card.addView(label(title, 10.5f, ink, true).apply { maxLines = 1; ellipsize = android.text.TextUtils.TruncateAt.END; setPadding(dp(2), dp(6), 0, 0) }); card.addView(label(artist, 9f, muted)); parent.addView(card, LinearLayout.LayoutParams(dp(116), -1)) }
    private fun mixCard(parent: LinearLayout, title: String, start: Int, end: Int) { val card = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(5), 0, dp(5), 0); setOnClickListener { showLibrary() } }; val art = FrameLayout(this).apply { this.background = gradient(start, end, 17); addView(label("◉", 30f, Color.WHITE, true).apply { gravity = Gravity.CENTER }); addView(label("▶", 9f, blue, true).apply { gravity = Gravity.BOTTOM or Gravity.RIGHT; this.background = bg(Color.WHITE, 12); setPadding(dp(6), dp(5), dp(6), dp(5)) }) }; card.addView(art, LinearLayout.LayoutParams(dp(108), dp(108))); card.addView(label(title, 10.5f, ink, true).apply { setPadding(dp(2), dp(7), 0, 0) }); parent.addView(card, LinearLayout.LayoutParams(0, -1, 1f)) }
    private fun miniSuggestion(): LinearLayout = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(9), dp(6), dp(7), dp(6)); this.background = bg(alpha(Color.WHITE, 235), 18, alpha(Color.WHITE, 245)); val art = FrameLayout(this@NeonActivity).apply { this.background = gradient(blue, indigo, 13); addView(label("◉", 19f, Color.WHITE, true).apply { gravity = Gravity.CENTER }) }; addView(art, LinearLayout.LayoutParams(dp(48), dp(48))); val info = LinearLayout(this@NeonActivity).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(10), 0, 0, 0) }; info.addView(label("Glass...", 11f, ink, true)); info.addView(label("Astral Pulse", 9f, muted)); addView(info, LinearLayout.LayoutParams(0, -2, 1f)); addView(label("▶", 14f, blue, true).apply { setOnClickListener { playing = true; updateMini() } }, LinearLayout.LayoutParams(dp(36), dp(42))) }

    private fun showSearch() { currentTab = 1; headerTitle.text = "Search"; val c = column(); c.addView(label("Find your music", 26f, ink, true)); val input = EditText(this).apply { hint = "Songs, artists, albums..."; singleLine = true; this.background = bg(Color.WHITE, 18, alpha(blue, 45)); setPadding(dp(14), 0, dp(14), 0) }; c.addView(input, LinearLayout.LayoutParams(-1, dp(52)).apply { topMargin = dp(14); bottomMargin = dp(14) }); section(c, "Browse"); arrayOf("Songs", "Albums", "Artists", "Playlists").forEach { b -> c.addView(label("  $b", 14f, ink, true).apply { gravity = Gravity.CENTER_VERTICAL; this.background = bg(alpha(Color.WHITE, 225), 16); setPadding(dp(10), 0, 0, 0); setOnClickListener { showHome() } }, LinearLayout.LayoutParams(-1, dp(52)).apply { bottomMargin = dp(8) }) }; clearAndShow(scrollPage(c)); refreshNav() }
