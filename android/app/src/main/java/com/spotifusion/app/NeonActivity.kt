...
open class NeonActivity : AppCompatActivity() {
    ...
    private fun search() { currentTab = 1; val c = base(); header(c, "SEARCH", "FIND YOUR NEXT TRACK"); val e = EditText(this).apply { hint = "Search songs, artists, albums..."; setHintTextColor(muted); setTextColor(white); textSize = 14f; singleLine = true; setPadding(dp(18), 0, dp(18), 0); background = box(panel, 22) }; c.addView(e, LinearLayout.LayoutParams(-1, dp(52))); c.addView(tv("DISCOVER", 16f, white, true).apply { setPadding(0, dp(22), 0, dp(10)) }); card(c, "NEON HITS", "Trending sounds and new releases", cyan); card(c, "LOCAL MUSIC", "Music available on your device", purple); card(c, "RECENT SEARCHES", "Your latest discoveries", pink); show(c) }
