...
open class NeonActivity : AppCompatActivity() {
    ...
    private fun buildShell() {
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setBackgroundColor(background); setPadding(dp(12), dp(8), dp(12), 0) }
    ...
    private fun topBar(): LinearLayout = LinearLayout(this).apply {
        ...
        addView(label("⌕", 25f, ink, true).apply { gravity = Gravity.CENTER; contentDescription = "Search"; isClickable = true; this.background = bg(alpha(surface, 225), 17, alpha(Color.WHITE, 230)); setOnClickListener { showSearch() } }, LinearLayout.LayoutParams(dp(44), dp(44)).apply { rightMargin = dp(5) })
        addView(label("●", 20f, ink, true).apply { gravity = Gravity.CENTER; contentDescription = "Account"; isClickable = true; this.background = bg(alpha(surface, 225), 17, alpha(Color.WHITE, 230)); setOnClickListener { showSettings() } }, LinearLayout.LayoutParams(dp(44), dp(44)))
    }

    private fun bottomNavigation(): LinearLayout = LinearLayout(this).apply {
        orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER; setPadding(dp(5), dp(4), dp(5), dp(6)); this.background = bg(alpha(Color.WHITE, 242), 23, alpha(Color.WHITE, 250)); elevation = dp(5).toFloat()
        val items = arrayOf("⌂" to "Home", "⌕" to "Search", "▣" to "Library", "≋" to "Equalizer", "⚙" to "Settings")
        items.forEachIndexed { index, pair ->
            val selected = index == currentTab
            val cell = LinearLayout(this@NeonActivity).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER; setPadding(dp(2), dp(3), dp(2), dp(2)); contentDescription = pair.second; isClickable = true; isFocusable = true; if (selected) this.background = bg(alpha(blue, 25), 16); setOnClickListener { currentTab = index; when (index) { 0 -> showHome(); 1 -> showSearch(); 2 -> showLibrary(); 3 -> showEqualizer(); 4 -> showSettings() }; refreshNav() } }
    ...
    private fun miniPlayer(): LinearLayout = LinearLayout(this).apply {
        ...
        addView(label("♡", 22f, muted).apply { gravity = Gravity.CENTER; contentDescription = "Favorite"; isClickable = true }, LinearLayout.LayoutParams(dp(38), dp(50)))
        miniPlay = label(if (playing) "Ⅱ" else "▶", 18f, ink, true).apply { gravity = Gravity.CENTER; contentDescription = "Play or pause"; isClickable = true; setOnClickListener { playing = !playing; updateMini() } }; addView(miniPlay, LinearLayout.LayoutParams(dp(43), dp(50)))
        addView(label("≋", 20f, ink, true).apply { gravity = Gravity.CENTER; contentDescription = "Now playing"; isClickable = true; setOnClickListener { showNowPlaying() } }, LinearLayout.LayoutParams(dp(40), dp(50)))
    }
    ...
    private fun scrollPage(content: View): ScrollView = ScrollView(this).apply { isFillViewport = true; overScrollMode = View.OVER_SCROLL_IF_CONTENT_SCROLLS; setBackgroundColor(background); addView(content, ViewGroup.LayoutParams(-1, -1)) }
    ...
    private fun miniSuggestion(): LinearLayout = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(9), dp(6), dp(7), dp(6)); this.background = bg(alpha(Color.WHITE, 235), 18, alpha(Color.WHITE, 245)); val art = FrameLayout(this@NeonActivity).apply { this.background = gradient(blue, indigo, 13); addView(label("◉", 19f, Color.WHITE, true).apply { gravity = Gravity.CENTER }) }; addView(art, LinearLayout.LayoutParams(dp(48), dp(48))); val info = LinearLayout(this@NeonActivity).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(10), 0, 0, 0) }; info.addView(label("Glass Architecture", 11.5f, ink, true)); info.addView(label("Nordic Echoes", 9.5f, muted).apply { setPadding(0, dp(3), 0, 0) }); addView(info, LinearLayout.LayoutParams(0, -1, 1f)); addView(label("▶", 15f, Color.WHITE, true).apply { gravity = Gravity.CENTER; contentDescription = "Play suggestion"; isClickable = true; this.background = bg(blue, 15); setPadding(dp(9), dp(8), dp(9), dp(8)); setOnClickListener { selectTrack("Glass Architecture", "Astral Pulse") } }, LinearLayout.LayoutParams(dp(38), dp(38))) }

    private fun showSearch() {
        currentTab = 1; headerTitle.text = "Search"; val c = column(); c.addView(label("Search", 26f, ink, true).apply { setPadding(dp(2), dp(7), 0, dp(14)) })
        val searchBox = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(14), 0, dp(10), 0); this.background = bg(alpha(Color.WHITE, 235), 19, alpha(Color.WHITE, 245)); elevation = dp(2).toFloat() }; searchBox.addView(label("⌕", 23f, muted, true)); val input = EditText(this).apply { hint = "Search songs, artists, albums..."; textSize = 13.5f; setTextColor(ink); setHintTextColor(muted); isSingleLine = true; background = null; setPadding(dp(9), 0, 0, 0) }; searchBox.addView(input, LinearLayout.LayoutParams(0, dp(54), 1f)); searchBox.addView(label("⌁", 19f, muted, true).apply { gravity = Gravity.CENTER }); c.addView(searchBox, LinearLayout.LayoutParams(-1, dp(54)).apply { bottomMargin = dp(15) })
        section(c, "Recent Searches", "Clear all"); arrayOf("glass architecture", "astral pulse", "nordic echoes", "midnight drive", "ocean bloom").forEach { query -> val row = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(12), dp(7), dp(10), dp(7)); this.background = bg(alpha(Color.WHITE, 220), 16); setOnClickListener { input.setText(query); input.setSelection(input.length()) } }; row.addView(label("◷", 18f, muted)); row.addView(label(query, 11.5f, ink, true).apply { setPadding(dp(10), 0, 0, 0) }, LinearLayout.LayoutParams(0, dp(50), 1f)); row.addView(label("×", 18f, muted)); c.addView(row, LinearLayout.LayoutParams(-1, dp(64)).apply { bottomMargin = dp(7) }) }
        clearAndShow(scrollPage(c)); refreshNav()
    }

    private fun selectTrack(title: String, artist: String) { currentSong = title; currentArtist = artist; playing = true; updateMini(); showNowPlaying() }

    private fun showNowPlaying() {
        headerTitle.text = "Now Playing"; val c = column().apply { gravity = Gravity.CENTER_HORIZONTAL }; c.addView(LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; addView(label("⌄", 22f, ink, true).apply { contentDescription = "Back"; isClickable = true; setOnClickListener { showHome() } }); addView(label("Now Playing", 14f, ink, true).apply { gravity = Gravity.CENTER }, LinearLayout.LayoutParams(0, dp(38), 1f)); addView(label("⋮", 22f, ink, true).apply { contentDescription = "More options"; isClickable = true }) }, LinearLayout.LayoutParams(-1, dp(42)).apply { bottomMargin = dp(4) })
        val disc = FrameLayout(this).apply { this.background = bg(Color.rgb(252, 253, 255), 999, alpha(blue, 90)); elevation = dp(5).toFloat() }; val vinyl = FrameLayout(this).apply { this.background = bg(Color.rgb(25, 27, 34), 999, alpha(Color.WHITE, 40)); addView(label("◉", 42f, blue, true).apply { gravity = Gravity.CENTER }) }; disc.addView(vinyl, FrameLayout.LayoutParams(dp(214), dp(214), Gravity.CENTER)); c.addView(disc, LinearLayout.LayoutParams(dp(274), dp(274)).apply { topMargin = dp(8); bottomMargin = dp(22) })
        c.addView(label(currentSong, 22f, ink, true).apply { gravity = Gravity.CENTER }); c.addView(label("$currentArtist · Nordic Echoes", 12f, muted).apply { gravity = Gravity.CENTER; setPadding(0, dp(6), 0, dp(18)) }); val progress = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(3), 0, dp(3), 0) }; val bar = SeekBar(this).apply { max = 100; this.progress = 42; progressTintList = android.content.res.ColorStateList.valueOf(blue); thumbTintList = android.content.res.ColorStateList.valueOf(blue); contentDescription = "Playback position" }; progress.addView(bar, LinearLayout.LayoutParams(-1, dp(28))); val times = LinearLayout(this).apply { addView(label("1:28", 9.5f, muted), LinearLayout.LayoutParams(0, -2, 1f)); addView(label("3:42", 9.5f, muted).apply { gravity = Gravity.RIGHT }) }; progress.addView(times); c.addView(progress, LinearLayout.LayoutParams(-1, dp(47)))
        val actions = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER; setPadding(0, dp(7), 0, 0) }; actions.addView(label(if (shuffle) "🔀" else "⇄", 18f, if (shuffle) blue else muted).apply { contentDescription = "Shuffle"; isClickable = true; setOnClickListener { shuffle = !shuffle; setTextColor(if (shuffle) blue else muted) } }, LinearLayout.LayoutParams(dp(45), dp(55))); actions.addView(label("|◀", 21f, ink, true).apply { gravity = Gravity.CENTER; contentDescription = "Previous"; isClickable = true }, LinearLayout.LayoutParams(dp(50), dp(55))); actions.addView(label(if (playing) "Ⅱ" else "▶", 24f, Color.WHITE, true).apply { gravity = Gravity.CENTER; contentDescription = "Play or pause"; isClickable = true; this.background = bg(blue, 20); setOnClickListener { playing = !playing; updateMini(); text = if (playing) "Ⅱ" else "▶" } }, LinearLayout.LayoutParams(dp(64), dp(58))); actions.addView(label("▶|", 21f, ink, true).apply { gravity = Gravity.CENTER; contentDescription = "Next"; isClickable = true }, LinearLayout.LayoutParams(dp(50), dp(55))); actions.addView(label("↻", 21f, if (repeat) blue else muted, true).apply { contentDescription = "Repeat"; isClickable = true; setOnClickListener { repeat = !repeat; setTextColor(if (repeat) blue else muted) } }, LinearLayout.LayoutParams(dp(45), dp(55))); c.addView(actions); clearAndShow(scrollPage(c))
    }

    private fun showLibrary() { currentTab = 2; headerTitle.text = "Your Library"; val c = column(); c.addView(label("Your Library", 26f, ink, true).apply { setPadding(dp(2), dp(7), 0, dp(14)) }); val filters = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }; arrayOf("Playlists", "Songs", "Artists", "Albums").forEachIndexed { i, name -> filters.addView(label(" $name ", 10.5f, if (i == 0) blue else muted, true).apply { gravity = Gravity.CENTER; isClickable = true; isFocusable = true; contentDescription = name; this.background = bg(if (i == 0) alpha(blue, 28) else alpha(Color.WHITE, 220), 13); setPadding(dp(4), dp(8), dp(4), dp(8)); setOnClickListener { Toast.makeText(this@NeonActivity, "$name selected", Toast.LENGTH_SHORT).show() } }, LinearLayout.LayoutParams(0, dp(36), 1f).apply { setMargins(dp(2), 0, dp(2), 0) }) }; c.addView(filters, LinearLayout.LayoutParams(-1, dp(38)).apply { bottomMargin = dp(14) }); libraryItem(c, "♥", "Liked Songs", "432 songs", blue); libraryItem(c, "◉", "Chill Vibes", "by Kruthagna · 50 songs", indigo); libraryItem(c, "◉", "Night Drive", "by Kruthagna · 42 songs", cyan); libraryItem(c, "✦", "Focus Flow", "by Kruthagna · 38 songs", pink); libraryItem(c, "◉", "Retro Wave", "by Kruthagna · 55 songs", orange); libraryItem(c, "≋", "Workout Hits", "by Kruthagna · 60 songs", blue); clearAndShow(scrollPage(c)); refreshNav() }
